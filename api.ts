...
export const getData = async (endpoint: string, params: Record<string, string | number> = {}) => {
    try {
      const token = await AsyncStorage.getItem('authToken');
      const url = new URL(`${API_BASE_URL}/${endpoint}`);
  
      Object.keys(params).forEach((key) =>
        url.searchParams.append(key, String(params[key])) // Chuyển giá trị thành string
      );
